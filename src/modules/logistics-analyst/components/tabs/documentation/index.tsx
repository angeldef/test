...
export const DocsTab = ({ documentation, proposalId, reasons, getData }: Props) => {
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: (params) => {
        const {
          row: { status, reason, locked, url, mustApproveAnalyst, approvedByAnalyst },
        } = params;

        return (
          <>
            {url && mustApproveAnalyst && (
              <div style={{ display: 'flex', gap: '10px' }} className={styles.statusActions}>
                <div
                  className={`${approvedByAnalyst ? styles.accepted : null}`}
  ...
  const updateApprovalRequest = (index: number, approved: boolean, reasonReturned: string) => {
    ...
    array[index] = { approved, key, reasonReturned };
  ...
  const onChangeStatus = (row: Document, status: DocStatus) => {
    const { index } = row;
    const array = [...rows];
    array[index] = { ...array[index], status, approvedByAnalyst: status === DocStatus.ACCEPTED };
    setRows(array);
  ...
  const fillTable = async () => {
    let dataTable = documentation.map((e: any, i) => {
      ...
      return {
        ...
        approvedByNegociator: negotiatiorApproval?.approved,
        approvedByAnalyst: analystApproval
          ? analystApproval?.approved
    ...
    dataTable = dataTable.map((e) => (e.approvedByAnalyst === true || e.approvedByAnalyst === false ? { ...e, locked: true } : e));
  ...
  const confirmSave = async () => {
    ...
    setHasRejected(!!approvalRequest.some((item) => item?.approved === false));
    setOpenConfirm(true);
    setLoading(false);
  };
  ...
  const save = async () => {
    setLoading(true);
    const { data: resp } = await logisticsAnalystService.docApprovals(
      approvalRequest.filter((item) => item.key),
      proposalId
    );
  ...
  useEffect(() => {
    ...
    const array = documentation.map((e) => {
      const { approvedBy, analystApproval } = e ?? {};
      if ((e?.status === DocStatus.ACCEPTED || e?.status === DocStatus.REJECTED) && e.allowModify !== false) {
        if (e?.status === DocStatus.REJECTED) return {};
        return {
          approved: e?.status === DocStatus.ACCEPTED,
          key: e?.key,
          reasonReturned: e?.reasonReturned,
        };
      } else return !approvedBy?.includes('ANALISTA') || analystApproval?.approved ? {} : null;
    }) as ApprovalRequest[];
    setApprovalRequest(array);

    setCompleted(
      !documentation.some((item) => {
        const { analystApproval, approvedBy, status } = item ?? {};
        const { approved } = analystApproval ?? {};
        const mustApproveAnalyst = approvedBy?.includes('ANALISTA');
        return (
          (mustApproveAnalyst && (!analystApproval || (analystApproval && approved !== true && approved !== false))) ||
